...
/**
 * \file sinh_v2_tiling.cpp
 * \brief SinhV2 Tiling implementation (arch32 - Ascend910B)
 *
 * Computes tiling parameters for SinhV2 operator:
...
#include "../op_kernel/sinh_v2_tiling_data.h"
#include "../op_kernel/sinh_v2_tiling_key.h"
...
static ge::graphStatus SinhV2TilingFunc(gert::TilingContext* context)
{
    // 1. Get platform info
    uint64_t ubSize;
    int64_t coreNum;
    ...
    // 4. Set TilingData
    SinhV2TilingData* tiling = context->GetTilingData<SinhV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(SinhV2TilingData), 0, sizeof(SinhV2TilingData)) != EOK,
    ...
    if (totalNum == 0) {
        ...
        context->SetTilingKey(GET_TPL_TILING_KEY(SINHV2_TPL_SCH_MODE_0));
        return ge::GRAPH_SUCCESS;
    }
    ...
    if (dtype == ge::DT_FLOAT) {
        ...
        context->SetTilingKey(GET_TPL_TILING_KEY(SINHV2_TPL_SCH_MODE_1));
    } else {
        OP_LOGE(context, "SinhV2: unsupported dtype");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForSinhV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

struct SinhV2CompileInfo {};

IMPL_OP_OPTILING(SinhV2).Tiling(SinhV2TilingFunc).TilingParse<SinhV2CompileInfo>(TilingParseForSinhV2);
